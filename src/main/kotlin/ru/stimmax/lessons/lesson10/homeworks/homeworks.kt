...
// и выведи в цикле результат деления ключа на значение. Не забудь обработать деление на 0 (в этом случае выведи слово “бесконечность”)
    ...
    for ((k, v) in mmap4) {
        if (v == 0) {
            println("Бесконечность"); continue
        }
        println(k / v)
    }
...
// у которого пара будет содержать цифру 5 в качестве первого или второго значения.
    val map11: Map<Pair<Int, Int>, String> = mapOf((1 to 1) to "Куку", (5 to 1) to "Попався!", (2 to 5) to "Повався")
    for ((k, v) in map11) {
    ...
    val a: MutableMap<String, MutableList<String>> =
        mutableMapOf("Терри Пратчет" to mutableListOf("Цвет волшебства", "Последний континент"))
